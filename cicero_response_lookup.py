...
def main() -> None:
  r = st.radio("Search method", ["Substring", "Case-insensitive substring"])[0] #this [0] just saves us verbosity later
  ...
  # I also considered implementing the option of exact full-text match but then we'd have to parse the "responsegiven".
  ...
  if query:
    st.write(
      sql_call(
        f"SELECT * from cicero.default.activity_log WHERE responsegiven {'I' if r=='C' else ''}LIKE CONCAT('%', %(query)s, '%')",
        {'query': query}
      )
    )
