...
with st.sidebar:
  ...
  with st.expander("Advanced options (not yet hooked up)"):
    ...
    a = st.empty()
    top_k = a.number_input("top_k:", min_value=1, value=st.session_state.current_preset["top_k"], format='%d', step=1, help="The number of highest probability vocabulary tokens to keep for top-k-filtering. In other words: how many likely words the model will consider.")
