...
if chang_mode:
  with st.expander(r"$\textsf{\Large NEWS HEADLINES}$"):
    semantic_query = st.text_input(r"Semantic Search <br> <small>*Returns headlines matching the meaning of the search terms, not necessarily exact matches. Must hit Enter.*</small> <br>  <small>*Example: searching for `border' will also return headlines for 'immigration', 'migrants', 'border crossings', 'deportation', etc.*</small>", key="semantic_query")
