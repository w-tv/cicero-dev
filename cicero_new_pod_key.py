...
with c[3]:
  ...
  if st.button("update the activity log retroactively with that one new email and pod") and one_new_email and one_new_pod:
    sql_call("UPDATE cicero.default.activity_log SET user_pod = :pod WHERE user_email ilike :email", {"email": one_new_email, "pod": one_new_pod})
...
st.write(f"""activity log entries where the pod is NULL, suggesting you need to run the retroactive application (above) if there are any: ***{sql_call("SELECT count(*) FROM cicero.default.activity_log WHERE user_pod IS NULL")[0][0]}***""")
st.write(f"""activity log entries where the pod is "Pod unknown", suggesting you need to run the retroactive application (above) if there are any: ***{sql_call("SELECT count( distinct user_email) FROM cicero.default.activity_log WHERE user_pod = 'Pod unknown'")[0][0]}***""")
activity_log_results = sql_call("SELECT DISTINCT user_email, user_pod FROM cicero.default.activity_log")
