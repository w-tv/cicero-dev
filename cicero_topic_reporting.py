...
def main() -> None:
  ...
  with col3:
    project_type = st.multiselect("Project Type", ["Text Message", "Email"], help="This control allows you to filter on the project type, between email and text message. If Both selected, no filtering will be done.\n\n Internally, the filtering is done based on whether the project_type begins with \"\", \"Text Message\", or \"Email\".")
    project_type = "Both" if len(project_type)!=1 else project_type[0]
    project_string = {"Both": "true", "Text Message": "project_type like 'Text Message%'", "Email": "project_type like 'Email%'"}[project_type]
  with col4:
    ...
    askgoal = "Both" if len(askgoal)!=1 else askgoal[0]
  ...
  day_data_per_topic = sql_call(f"""WITH stats(date, funds, topic) AS (SELECT SEND_DATE, SUM(TV_FUNDS), Hooks FROM hook_reporting.default.hook_data_prod WHERE {project_string} and account_name in {to_sql_tuple_string(accounts)} and hooks in {to_sql_tuple_string(topics)} and {askgoal_string} and SEND_DATE >= NOW() - INTERVAL 30 DAY and SEND_DATE < NOW() and Hook_Bool=true and {search_string} GROUP BY SEND_DATE, Hooks) SELECT date, funds, topic from stats""", {"regexp": search})
