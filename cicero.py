...
with Profiler():
  ...
  if ssget("activity_log_payload"):
    ...
    sql_call_cacheless(
      ...
      (timestamp,           user_email, user_pod,                               prompter_or_chatbot,  prompt_sent,  response_given,  model_name,  model_url,  model_parameters,  system_prompt,  base_url, user_feedback,  user_feedback_satisfied,  used_similarity_search_backup,  hit_readlink_time_limit, pii_concern) SELECT
      current_timestamp(), :user_email, COALESCE(tmp.user_pod, 'Pod unknown'), :prompter_or_chatbot, :prompt_sent, :response_given, :model_name, :model_url, :model_parameters, :system_prompt, :base_url, :user_feedback, :user_feedback_satisfied, :used_similarity_search_backup, :hit_readlink_time_limit, :pii_concern
