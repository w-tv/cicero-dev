...
with Profiler():
  ...
  if iap_jwt := st.context.headers.get("X-Goog-Iap-Jwt-Assertion"):
    try:
      decoded_jwt = id_token.verify_token(iap_jwt, requests.Request(), audience=st.secrets["aud"], certs_url="https://www.gstatic.com/iap/verify/public_key") #type: ignore[no-untyped-call] # There is no vendor providing comprehensive type signatures for this library (it doesn't have them by default and google-auth-stubs isn't comprehensive enough), so we just ignore the fact that it's an untyped call, which mypy --strict otherwise complains about (for good reason). Oh well!
